...
class NotificationTest(APITestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.user_manager = UserManager()
        cls.url='/api/get-notifications/'

        cls.user = cls.user_manager.create(user_custom_name="user",username="unittest_user",password="887788uuy")
        cls.friend_1 = cls.user_manager.create(user_custom_name="friend",username="unittest_friend",password="885485tghq")
        cls.notification = NotificationFactory(user=cls.user.user)
        cls.notification_content = NotificationContentFactory(notification=cls.notification)
        cls.notification_deleted= NotificationFactory(user=cls.user.user,is_deleted=True)
        cls.notification_deleted_content = NotificationContentFactory(notification=cls.notification_deleted,text_content='deleted message')
        cls.notification_friend = NotificationFactory(user=cls.friend_1.user)
        cls.notification_content_friend = NotificationContentFactory(notification=cls.notification_friend)
    def setUp(self):
        self.client = APIClient()
        self.client.force_login(self.user.user)
    ...
    def test_get_notifications_success(self):
        ...
        print(response.content)
        self.assertEqual(response.status_code,200)
    def test_get_notifications_failed_on_no_user(self):
        self.client.logout()
        response = self.client.get(self.url)
        print(response.content)
